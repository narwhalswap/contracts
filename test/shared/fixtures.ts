...
import NarwhalswapFactory from '../../build/NarwhalswapFactory.json'
import INarwhalswapPair from '../../build/INarwhalswapPair.json'
import NarwhalswapRouter from '../../build/NarwhalswapRouter.json'
...
export async function factoryFixture(_: Web3Provider, [wallet]: Wallet[]): Promise<FactoryFixture> {
  const factory = await deployContract(wallet, NarwhalswapFactory, [wallet.address], overrides)
  return { factory }
}
...
export async function pairFixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<PairFixture> {
  ...
  const pair = new Contract(pairAddress, JSON.stringify(INarwhalswapPair.abi), provider).connect(wallet)
  ...
  const WBNBPair = new Contract(WBNBPairAddress, JSON.stringify(INarwhalswapPair.abi), provider).connect(wallet)
...
export async function routerFixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<RouterFixture> {
  ...
  const router = await deployContract(wallet, NarwhalswapRouter, [factory.address, WBNB.address], overrides)
